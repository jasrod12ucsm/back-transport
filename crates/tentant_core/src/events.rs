...
use futures::StreamExt; // Para usar .next() en Stream
...
// Implementar From para errores de NATS convirtiendo a String
...
pub enum TenantEvent {
    Created(TenantCreatedEvent),
    Updated(TenantUpdatedEvent),
    Deactivated(TenantDeactivatedEvent),
}
...
impl TenantEventPublisher {
    ...
    /// Publica evento TenantCreated
    pub async fn publish_created(&self, event: TenantCreatedEvent) -> Result<(), EventError> {
        let subject = format!("{}.created", self.stream_name);
        self.publish(subject, TenantEvent::Created(event)).await
    }

    /// Publica evento TenantUpdated
    pub async fn publish_updated(&self, event: TenantUpdatedEvent) -> Result<(), EventError> {
        let subject = format!("{}.updated", self.stream_name);
        self.publish(subject, TenantEvent::Updated(event)).await
    }

    /// Publica evento TenantDeactivated
    pub async fn publish_deactivated(
        &self,
        event: TenantDeactivatedEvent,
    ) -> Result<(), EventError> {
        let subject = format!("{}.deactivated", self.stream_name);
        self.publish(subject, TenantEvent::Deactivated(event)).await
    }
        ...
        loop {
            match messages.next().await {
                Some(Ok(msg)) => {
                    ...
                    match event {
                        Ok(TenantEvent::Created(data)) => {
                            info!(tenant_id = %data.tenant_id, "Received TenantCreated event");

                            if let Err(e) = handler.on_tenant_created(&data).await {
                                error!(error = %e, "Failed to handle TenantCreated");
                            }
                        }
                        Ok(TenantEvent::Updated(data)) => {
                            info!(tenant_id = %data.tenant_id, "Received TenantUpdated event");

                            if let Err(e) = handler.on_tenant_updated(&data).await {
                                error!(error = %e, "Failed to handle TenantUpdated");
                            }
                        }
                        Ok(TenantEvent::Deactivated(data)) => {
                            info!(tenant_id = %data.tenant_id, "Received TenantDeactivated event");
