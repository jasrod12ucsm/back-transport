...
def category_boxplot_with_outliers(
    ...
    if df_clean.height == 0:
        return {"x": [], "meta": [], "leyend": [{"nombre": value_col, "color": _deterministic_rgb(value_col)}], "outliers": []}
    ...
    exprs= [pl.col(value_col).list.all().alias("outliers")]
    outliers = (
        joined.filter(
            (pl.col(value_col) < pl.col("lower")) | (pl.col(value_col) > pl.col("upper"))
        )
    ...
    final = final.with_columns([
        ...
        pl.col("outliers").fill_null(pl.lit([])).alias("outliers")
    ])

    # 8) Construir listas alineadas: meta, x y outliers
    meta: list[Any] = final[category_col].to_list()
